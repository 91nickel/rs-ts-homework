...
const normalizeData = (unnormalizedData: IPost[]): INormalizedData => {
    ...
    // Почему такая конструкция выпадает с ошибкой
    // A spread argument must either have a tuple type or be passed to a rest parameter.
    // const byId = Object.assign(...unnormalizedData.map((d: IPost) => ({[d.id]: d})))
    // console.log(byId)

    // А вот такая работает нормально
    // const byId2 = Object.assign(...[{}], ...unnormalizedData.map((d: IPost) => ({[d.id]: d})))
    // console.log(byId2)

    return {
        byId: unnormalizedData.reduce((agr: {}, cur: IPost) => ({...agr, [cur.id]: cur}), {}),
        allIds: unnormalizedData.map(d => d.id)
    }
};
